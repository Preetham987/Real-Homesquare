...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $query = "SELECT * FROM builders_table WHERE email = '$email'";
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        $storeToken = "UPDATE builders_table SET reset_token='$token', reset_token_expiry='$tokenExpiry' WHERE email='$email'";
        if (mysqli_query($conn, $storeToken)) {
            // Create reset link
            $resetLink = "https://rigvesoft.com/homesquare/real-estate-panels-2/builder_panel/confirm-password.php?token=$token";
