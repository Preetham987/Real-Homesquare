...
$builder_options = "";
$sql = "SELECT builder_name FROM builders_table";
$result = $conn->query($sql);

$builder_names = []; // Store builder names in an array
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $builder_names[] = htmlspecialchars($row['builder_name']); // Prevent XSS attacks
    }
}
                                    ...
                                    <div class="row clearfix">
                                        <div class="col-lg-6 col-md-6">
                                            ...
                                            <select class="form-control show-tick" name="builder_name" required>
                                                ...
                                                <?php
                                                foreach ($builder_names as $builder) {
                                                    $selected = (isset($project['builder_name']) && $project['builder_name'] == $builder) ? 'selected' : ''; // Retain selected builder per project
                                                    echo "<option value='$builder' $selected>$builder</option>";
                                                }
                                                ?>
                                            </select>
                                        </div>
                                        ...
                                        <?php
// Split the banner_image into parts: main and side images
$allImages = !empty($project['banner_image']) ? explode(',', $project['banner_image']) : [];
$mainImage = $allImages[0] ?? '';
$sideImages = array_slice($allImages, 1);
?>
...
<div class="col-sm-6">
    ...
    <div class="form-group">
        ...
        <?php if (!empty($mainImage)): ?>
            ...
            <p class="mt-2 text-muted"><?= basename($mainImage) ?></p>
        <?php endif; ?>

        <label class="mt-3">Side Banner Image(s)</label>
        ...
        <?php if (!empty($sideImages)): ?>
            <br>
            <div class="row mt-3">
                <?php foreach ($sideImages as $sideImage): ?>
                    <div class="col-6 mb-3">
                        <img src="<?= htmlspecialchars($sideImage) ?>"
                             alt="Side Banner Image"
                             class="img-fluid rounded"
                             style="width: 100%; height: auto; border: 1px solid #ccc; padding: 5px;">
                        <p class="text-muted small text-center"><?= basename($sideImage) ?></p>
                    </div>
                <?php endforeach; ?>
            </div>
        <?php endif; ?>

        <!-- JS validation for main image -->
        <script>
            ...
            <div class="col-lg-6 col-md-4 col-sm-6">
                ...
                <?php if (!empty($project_images[$i][$j])): ?>
                    ...
                    <div class="mt-2 preview-wrapper" id="preview_<?= $i ?>_<?= $j ?>">
                        <img src="<?= htmlspecialchars($project_images[$i][$j]) ?>" alt="Project Image" class="img-fluid rounded" style="max-width: 100px; border: 1px solid #ccc; padding: 4px;" />
                            ...
                            <?php if (!empty($bank_documents[$index])): ?>
                                <div class="mt-2">
                                    ...
                                    <?php if (in_array(strtolower($ext), ['jpg', 'jpeg', 'png'])): ?>
                                        <img src="<?= $doc_path ?>" alt="Bank Document" class="img-fluid rounded" 
